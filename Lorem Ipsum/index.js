const text = [
  ...
  const generate = document.getElementsByName('generate')[0]

  generate.addEventListener('click', () => {
    let num = document.querySelector('input').value
    if(+num === 1 || +num > 9){
        rand = Math.floor(Math.random() * text.length)
        document.querySelector('form').querySelector('article').innerHTML = `<p>${text[rand]}</p>`
    }
    else{
        article = ''
        for(let i = 0; i < +num; i++){
            article += `<p>${text[i]}</p>`
            document.querySelector('form').querySelector('article').innerHTML = article
        }
    }
  })